...
public class ThoughtLoomBlockEntityRenderer implements BlockEntityRenderer<ThoughtLoomBlockEntity> {
    ...
    @Override
    public void render(ThoughtLoomBlockEntity te, float p_112308_, PoseStack ms,
        ...
        if(!stack.isEmpty()) {
            ms.pushPose();
            ms.translate(0.5, 1.25 + 0.015 * Math.cos(0.15f * te.getLevel().getGameTime()), 0.5);
            ms.mulPose(Axis.YP.rotation(te.getLevel().getGameTime() / 24.8525f));
        ...
        if(!stack1.isEmpty()) {
            ...
            ms.mulPose(Axis.YP.rotationDegrees((float)-(360 + 0.85*te.getLevel().getGameTime())));
        ...
        if(!stack2.isEmpty()) {
            ...
            ms.mulPose(Axis.YP.rotationDegrees((float)-(360 + 0.65*te.getLevel().getGameTime())));
        ...
        if(!stack3.isEmpty()) {
            ...
            ms.mulPose(Axis.YP.rotationDegrees((float)-(360 + 0.45*te.getLevel().getGameTime())));

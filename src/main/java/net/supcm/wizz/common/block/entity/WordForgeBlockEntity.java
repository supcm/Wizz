...
public class WordForgeBlockEntity extends BlockEntity {
    public WordForgeBlockEntity(BlockPos p_155229_, BlockState p_155230_) {
        super(BlockEntities.WORD_FORGE.get(), p_155229_, p_155230_);
    }
    ...
    public void getEnchLevel(int tier) {
        ...
        int mult = WizzModConfig.ENCHANTING_MULT.get() / ench.getMaxLevel();
        ...
        if(count > ench.getMaxLevel() && ench.getMaxLevel() != 1)
            enchLevel = (int) (WizzModConfig.ENCHANTING_MULT.get()
                    + mult * count * WizzModConfig.ENCHANTING_EASE.get());
        else {
            if (ench.getMaxLevel() == 1) count = 1;
            enchLevel = mult * count;
        }
    }
    ...
    public Enchantment getEnchantmentT3() {
        ...
        ResourceLocation rl1 = Items.getResourceLocation(handler.getStackInSlot(1)
                .getItem());
        ResourceLocation rl2 = Items.getResourceLocation(handler.getStackInSlot(2)
                .getItem());
        if(EnchantmentsHandler.T3_MAP.keySet().contains(rl.getPath() + "_" + rl1.getPath() + "_" + rl2.getPath()))
            ench = EnchantmentsHandler.T3_MAP.get(rl.getPath() + "_" + rl1.getPath() + "_" + rl2.getPath());
        return ench;
    }
    public InteractionResult enchantBook(Player player, ItemStack handItem, int tier) {
        ...
        if(count > ench.getMaxLevel()) {
            if(level.getRandom().nextInt(17-count) == 0) {
                if(!warranty) {
                    warranty = true;
                    curse = true;
                } else warranty = false;
        ...
        if (player.experienceLevel < enchLevel && !player.isCreative()){
            player.displayClientMessage(Component.translatable("enchanting.notenoughxp"), true);
        ...
        if(handItem.getItem() == net.minecraft.world.item.Items.BOOK){
            ...
            if(curse)
                EnchantedBookItem.addEnchantment(stack,
                        new EnchantmentInstance(EnchantmentsHandler.getRandomCurse(level.random), count));
            player.addItem(stack);
        } else {
            Map<Enchantment, Integer> data = EnchantmentHelper.getEnchantments(handItem);
            data.putIfAbsent(ench, count);
            if(curse)
                data.putIfAbsent(EnchantmentsHandler.getRandomCurse(level.random), count);
